    ...
    if($_SESSION['timeout'] + $timeout < time()){
        ...
        if(check_id_product($id)){		
            ...
            if(!empty($row['products_image']) ){
                $imagesrc = "<img src='"._UPLOAD_IMG.$row['products_image']."' width='170px' height='120px'/>";	
            }	
            
            if(!empty($row['products_image_first']) ){
                $imagesrc_first = "<img src='"._UPLOAD_IMG.$row['products_image_first']."' width='80px' height='60px'/>";                
            }	
            
            if(!empty($row['products_image_second']) ){
                $imagesrc_second = "<img src='"._UPLOAD_IMG.$row['products_image_second']."' width='80px' height='60px'/>";                
            }	
            
            if(!empty($row['products_image_third']) ){
                $imagesrc_third = "<img src='"._UPLOAD_IMG.$row['products_image_third']."' width='80px' height='60px'/>";                
            }	
            
            if(!empty($row['products_image_fourth']) ){
                $imagesrc_fourth = "<img src='"._UPLOAD_IMG.$row['products_image_fourth']."' width='80px' height='60px'/>";
            }	            
            ...
            if(empty($error)){    
                ...
                if($mysql -> query()){	
                    ...
                    if($image){
                        //Update image
                        if($update == 1){
                            if(file_exists(_UPLOAD_IMG.$row['products_image'])){	                                
                        ...
                        if(move_uploaded_file($_FILES ["image"]["tmp_name"],_UPLOAD_IMG_THUMB.$image)){
                            @imagejpeg(resize(_UPLOAD_IMG_THUMB.$image,1000,500),_UPLOAD_IMG.$image);
                            $sql = "UPDATE products
                    ...
                    if($image_first){
                        //Update image
                        if($update == 1){                                                       
                            if(file_exists(_UPLOAD_IMG.$row['products_image_first'])){
                                @unlink(_UPLOAD_IMG.$row['products_image_first']);
                                @unlink(_UPLOAD_IMG_THUMB.$row['products_image_first']);
                            }
                            $lastId = $id;
                        }	
                        ...
                        if(move_uploaded_file($_FILES ["image_first"]["tmp_name"],_UPLOAD_IMG_THUMB.$image_first)){
                            @imagejpeg(resize(_UPLOAD_IMG_THUMB.$image_first,1000,500),_UPLOAD_IMG.$image_first);
                            $sql = "UPDATE products
                    ...
                    if($image_second){
                        //Update image
                        if($update == 1){
                            if(file_exists(_UPLOAD_IMG.$row['products_image_second'])){
                                @unlink(_UPLOAD_IMG.$row['products_image_second']);
                                @unlink(_UPLOAD_IMG_THUMB.$row['products_image_second']);
                            }
                            $lastId = $id;
                        }	
                        ...
                        //Resize image_first and move to folder
                        if(move_uploaded_file($_FILES ["image_second"]["tmp_name"],_UPLOAD_IMG_THUMB.$image_second)){
                            @imagejpeg(resize(_UPLOAD_IMG_THUMB.$image_second,1000,500),_UPLOAD_IMG.$image_second);
                            $sql = "UPDATE products
                    ...
                    if($image_third){
                        //Update image
                        if($update == 1){
                            if(file_exists(_UPLOAD_IMG.$row['products_image_third'])){
                                @unlink(_UPLOAD_IMG.$row['products_image_third']);
                                @unlink(_UPLOAD_IMG_THUMB.$row['products_image_third']);
                            }
                            $lastId = $id;
                        }	
                        ...
                        //Resize image_first and move to folder
                        if(move_uploaded_file($_FILES ["image_third"]["tmp_name"],_UPLOAD_IMG_THUMB.$image_third)){
                            @imagejpeg(resize(_UPLOAD_IMG_THUMB.$image_third,1000,500),_UPLOAD_IMG.$image_third);
                            $sql = "UPDATE products
                    ...
                    if($image_fourth){
                        //Update image
                        if($update == 1){
                            if(file_exists(_UPLOAD_IMG.$row['products_image_fourth'])){
                                @unlink(_UPLOAD_IMG.$row['products_image_fourth']);
                                @unlink(_UPLOAD_IMG_THUMB.$row['products_image_fourth']);
                            }
                            $lastId = $id;
                        }	
                        ...
                        //Resize image_first and move to folder
                        if(move_uploaded_file($_FILES ["image_fourth"]["tmp_name"],_UPLOAD_IMG_THUMB.$image_fourth)){
                            @imagejpeg(resize(_UPLOAD_IMG_THUMB.$image_fourth,1000,500),_UPLOAD_IMG.$image_fourth);
                            $sql = "UPDATE products
