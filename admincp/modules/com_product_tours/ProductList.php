<?php
    ...
    if(($_GET['action'] == 'del')&&isset($_GET['id'])){
        $id = intval($_GET['id']);
        delete_img_tours($id);
        delete_product_tours($id);
        if($_GET['order'] != ""){
            header('location:./?show=product_tours&order='.$_GET['order'].'&p='.intval($_GET['p']));
        }
        else{
    ...
    if(isset($_POST['btnencour'])) {
        $id = (count($_POST['chk']) > 0) ? implode($_POST['chk'],',') : '';
        $sql = "UPDATE products_tours SET p_encourage = 1 WHERE products_id in (".$id.")";
        $mysql -> setQuery($sql);
        $mysql -> query();
        if($_GET['order'] != ""){
            header('location:./?show=product_tours&order='.$_GET['order'].'&p='.intval($_GET['p']));
    ...
    if(isset($_POST['btnnew'])){
        $id = (count($_POST['chk']) > 0)?implode($_POST['chk'],','):'';
        $sql = "UPDATE products_tours SET p_new = 1 WHERE products_id in (".$id.")";
        $mysql -> setQuery($sql);
        $mysql -> query();
        if($_GET['order'] != ""){
            header('location:./?show=product_tours&order='.$_GET['order'].'&p='.intval($_GET['p']));
    ...
    if(isset($_POST['btnbest'])) {
        $id = (count($_POST['chk']) > 0)?implode($_POST['chk'],','):'';
        $sql = "UPDATE products_tours SET p_bestsell = 1  WHERE products_id in (".$id.")";
        $mysql -> setQuery($sql);
        $mysql -> query();
        if($_GET['order'] != ""){
            header('location:./?show=product_tours&order='.$_GET['order'].'&p='.intval($_GET['p']));
    ...
    for($i = $begin ; $i < $end ; ++$i){
        ...
        $imgSrc = _UPLOAD_IMG_TOURS.$img;
        $imgThumb = _UPLOAD_IMG_THUMB_TOURS.$img;
        if(!empty($img) and !checkExtentFile($img,$imgExtent )){
            $img ='<a href="'._UPLOAD_IMG_THUMB_TOURS.$img.'">File</a>';
        }
        else{
        ...
        $temp.= $xtemplate ->assign_vars($code_cats,array(
            ...
            'categories_id'	=> CatlistUpgrade_2_tours($row[$i]['upgrade_categories_id']),
            'product_image'	=> $img,
    ...
    $content = $xtemplate -> replace($content,array(
        ...
        'defaultText'		=> 'Tìm thông tin tours: ',
        'productKey'		=> $_SESSION['QUERY_KEY_S'],
        'lag_page'		=> $arr_lang['page'],
        'p'			=> ($_GET['p']=='')?'1':intval($_GET['p']),
        'oderby'		=> $_GET['order'],
        'page'			=> page_div("./?show=product_tours&p=%d_pg&order=".$_GET['order'], "10", ceil ( $numofpages ), $page),
        'lag_delete'		=> $arr_lang['del'],
        'lag_edit'		=> $arr_lang['edit'],
        'catlist'		=> cat_fillter_tours($_SESSION['CATID']),
