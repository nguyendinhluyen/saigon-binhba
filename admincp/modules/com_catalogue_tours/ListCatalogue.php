<?php
    ...
    if(($_GET['action']== 'del')&&(isset($_GET['id']))){
        ...
        delete_cats_tours(intval($_GET['id']));
        ...
        delete_cats_tours($id);
        if($_GET['order'] != ""){
            header('location:./?show=catalogue_tours&order='.$_GET['order'].'&p='.intval($_GET['p']));
        }
        else{
    ...
    //Phan trang
    $p_now = intval($_GET ['p']);
    $mysql -> setQuery("select categories_id from categories_tours where language ='".$_SESSION['lag']."'");// and level<>1");
    $mysql -> query();
    ...
    $sql = "select * from categories_tours where language ='".$_SESSION['lag']."' order by ".$orderby." limit ".$limitvalue.",".$pp;
    $mysql -> setQuery($sql);
    ...
    for($i = 0 ; $i < $n ; ++$i){
        ...
        $parent_name = get_parent_name_tours($row[$i]['parent_id']);
