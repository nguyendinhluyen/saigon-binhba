<?php    
    ...
    $productdetail = $xtemplate->load('tours_detail_bootstrap');
    $Product = new Product();
    ...
    $product_detail = $Product->getProductsToursByProductKey($product_key);
    ...
    if(empty($product_detail['product_detail']) && empty($product_detail['product_detail_tacdung'])
       ...
       &&empty($product_detail['product_detail_nhasanxuat']) && empty($product_detail['product_detail_xuatxu'])){
        $product_detail['product_detail'] = "Chưa có thông tin chi tiết về tour ". $product_detail['products_name'];
    }
    ...
    $product_detail['product_detail'] = str_replace ('size="2"', "style='font-size: 14px'", $product_detail['product_detail']);    
    ...
    // Relation tours
    $old_products = $Product->getProductsToursLimitOrderById(0,3);        
    for($i=0 ; $i<count($old_products) ; ++$i){
        $Category = new Category();
        $category_key = $Category->getCategoryKeyByProductToursID($old_products[$i]['products_id']);
        if($i == 0){
    ...
    $productdetail  = $xtemplate->replace($productdetail,array(
        ...
        'link_san-pham'                                 => $category_key."/".$product_detail['products_key'].".htmls",
        'khuyenmai'                                     => $khuyenmai,
